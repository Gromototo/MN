...
void algo_dijkstra (pgraphe_t g, int r)
{
  ...
  */
  g = chercher_sommet(g, r); //on est sur le sommet r
  if (g==NULL){
...
void imprimer_chemin_dijkstra(pgraphe_t g) {
  int cpt = 0;
    while(g->sommet_suivant!=NULL){
      printf("étiquette du noeud numero %d : %d et son poids associé : %d\n",cpt, g->label, g->poids_dijkstra);
      cpt++;
      g=g->sommet_suivant;
    }
}
